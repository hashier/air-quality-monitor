...
void SCD4XComponent::update() {
  ...
  this->set_timeout(wait_time, [this]() {
    ...
    if (!this->read_data(raw_read_status) || raw_read_status == 0x00) {
      this->status_set_warning();
      this->not_ready_count_++;
      ESP_LOGW(TAG, "Data not ready (consecutive=%u)", this->not_ready_count_);
      if (this->not_ready_count_ >= 2) {
        // Clean re-init per datasheet: stop -> wait 500ms -> start. A bare start
        // is a no-op if the sensor still thinks it's in periodic mode.
        ESP_LOGW(TAG, "Attempting recovery: stop -> start periodic measurement");
        if (!this->write_command(SCD4X_CMD_STOP_MEASUREMENTS)) {
          ESP_LOGE(TAG, "Recovery: stop command FAILED (I2C)");
        }
        this->set_timeout(500, [this]() {
          if (this->start_measurement_()) {
            ESP_LOGW(TAG, "Recovery: stop+start sequence issued");
          } else {
            ESP_LOGE(TAG, "Recovery: start command FAILED (I2C)");
          }
        });
    ...
    this->not_ready_count_ = 0;  // data was ready, clear the failure streak
